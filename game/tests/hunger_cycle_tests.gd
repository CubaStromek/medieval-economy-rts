...
static func _test_calendar_derived_loss(failures: Array[String]) -> void:
	...
	_check(world.hunger_loss_per_interval() == 8,
...
static func _test_real_daily_hunger_onset(failures: Array[String]) -> void:
	for initial: int in [2700, 1620]:
		...
		var expected: int = 4680 if initial == 2700 else 2520
...
static func _test_eta_accounts_for_tick_phase(failures: Array[String]) -> void:
	...
	for pair: Array in [[0, 20], [3, 17], [9, 11], [10, 20]]:
		...
		_check(int(world.hunger_status(worker)["remaining_to_hungry_ticks"]) == int(pair[1]),
			"Hunger ETA must account for the next decay at tick phase %d" % world.tick, failures)
	world.tick = 3
	worker["hunger"] = 361
	_check(int(world.hunger_status(worker)["remaining_to_hungry_ticks"]) == 7,
		"A partial final loss must round up to one scheduled decay", failures)
	worker["hunger"] = 6
	_check(int(world.hunger_status(worker)["remaining_to_starve_ticks"]) == 17,
		"Starvation ETA must use rounded-up losses and the current phase", failures)
	worker["hunger"] = 5
	_check(int(world.hunger_status(worker)["remaining_to_starve_ticks"]) == 7,
		"A worker with one loss remaining dies at the next scheduled check", failures)
	worker["hunger"] = 0
	_check(int(world.hunger_status(worker)["remaining_to_starve_ticks"]) == 0,
		"Already exhausted condition must have no future starvation delay", failures)
...
static func _test_status_boundaries(failures: Array[String]) -> void:
	...
	for pair: Array in [[2700, "Fed"], [1351, "Fed"], [1350, "Getting hungry"], [361, "Getting hungry"], [360, "Hungry"], [121, "Hungry"], [120, "Starving"], [0, "Starving"]]:
		...
		_check(status["state"] == pair[1] and int(status["hungry_at"]) == 360,
			"Satiety state must use the shared warning/hungry/critical boundaries at %d" % pair[0], failures)
...
static func _test_two_days_of_work_food_and_sleep(failures: Array[String]) -> void:
	for menu: Array in [["bread", "sausage"], ["bread", "wine", "fish"]]:
		...
		var ate_at_night: bool = false
		var slept: bool = false
		for _tick: int in range(DayCycle.TICKS_PER_DAY * 2):
			world.step_tick()
			...
			if eating and not was_eating:
				_check(world.tick - last_visit <= DayCycle.TICKS_PER_DAY,
					"A supplied working citizen must begin another meal within one calendar day", failures)
				last_visit = world.tick
				visits += 1
				ate_at_night = ate_at_night or world.is_night_rest_time()
			...
			if int(worker["home_id"]) != sawmill:
				...
				break
			was_eating = eating
		_check(visits >= 2 and world.tick - last_visit <= DayCycle.TICKS_PER_DAY,
			"Both two-course and three-course menus must support at least one visit per day", failures)
		...
		_check(ate_at_night and slept,
			"Nightly rest must allow a hungry citizen to eat and then resume sleeping", failures)
