...
static func _test_empty_satiety_and_real_weakness(failures: Array[String]) -> void:
	...
	_check(main.hud._satiety_label.text == "Sytost 0 % · Hladový" and main.hud._satiety_bar.value == 0.0
		and main.hud._satiety_timing.text.contains("Rezerva jídla: 70 / 70 min při 1×")
		and main.hud._satiety_timing.text.contains("Práce 100 % · chůze 100 %")
		and not main.hud._satiety_timing.text.contains("Pozor:"),
	...
	main._update_ui()
	_check(main.hud._satiety_label.text.contains("Zesláblý") and main.hud._satiety_fill.bg_color == Hud.satiety_color("Weakened")
		and main.hud._satiety_timing.text.contains("Rezerva jídla: 40 / 70 min při 1×")
		and main.hud._satiety_timing.text.contains("Práce 90 % · chůze 100 %")
		and not main.hud._satiety_timing.text.contains("Pozor:"),
	...
	main._update_ui()
	_check(main.hud._satiety_label.text.contains("Hladoví") and main.hud._satiety_fill.bg_color == Hud.satiety_color("Starving")
		and main.hud._satiety_timing.text.contains("Rezerva jídla: 5 / 70 min při 1×")
		and main.hud._satiety_timing.text.contains("Práce 80 % · chůze 100 %")
		and main.hud._satiety_timing.text.contains("bez jídla zbývá 5 min 00 s při 1× do smrti")
		and main.hud._satiety_timing.text.contains("2 min 30 s při rychlosti 2.0×"),
	...
	main._update_ui()
	_check(main.hud._satiety_timing.text == "V tomto scénáři je potřeba jídla vypnutá."
...
static func _test_seven_day_reserve_and_speed(failures: Array[String]) -> void:
	...
	for setting: Array in [[0.5, "140 min 00 s při rychlosti 0.5×"], [1.0, "70 min 00 s při rychlosti 1.0×"], [2.0, "35 min 00 s při rychlosti 2.0×"]]:
		main.simulation_speed = float(setting[0])
		main._update_ui()
		_check(main.hud._satiety_timing.text.contains("Rezerva jídla: 70 / 70 min při 1×")
			and main.hud._satiety_timing.tooltip_text.contains("Rezerva bez dalšího jídla: 70 min 00 s při 1×")
	...
	main._update_ui()
	_check(main.hud._satiety_timing.text.contains("Simulace je pozastavená")
		and main.hud._satiety_timing.tooltip_text.contains("Simulace je pozastavená")
		and not main.hud._satiety_timing.tooltip_text.contains("rychlosti 0.0×"),
...
static func _test_indoor_employee_nutrition(failures: Array[String]) -> void:
	...
	_check(main.selected_unit_id == id and main.hud._satiety_panel.visible
		and main.hud._satiety_timing.text.contains("Práce 90 %")
	...
	main._update_ui()
	_check(main.hud.production_detail_label.text.contains("Truhlář #%d • 1/1 • uvnitř" % id)
		and main.hud.production_detail_label.text.contains("Sytost 0 % · Zesláblý")
		and main.hud.production_detail_label.text.contains("Rezerva jídla: 40 / 70 min při 1×")
		and main.hud.production_detail_label.text.contains("Práce 90 % · chůze 100 %"),
...
static func _test_eating_rebuilds_reserve_progressively(failures: Array[String]) -> void:
	...
	world.step_tick()
	var main: Main = _main(world, id)
	_check(int(worker.get("meal_ticks_left", 0)) > 0 and main.hud._satiety_timing.text.contains("Jí Chléb")
		and main.hud._satiety_timing.text.contains("Rezerva jídla: 20 / 70 min při 1×")
		and main.hud._satiety_timing.tooltip_text.contains("Odpočet rezervy je zastavený během jídla")
		and not main.hud._satiety_timing.text.contains("Hlad odhadem"),
	...
	_check(int(worker["nutrition_deficit_ticks"]) > 28000 and int(worker["nutrition_deficit_ticks"]) < 30000
		and main.hud._satiety_timing.text.contains("Rezerva jídla: 22 / 70 min při 1×")
		and main.hud._satiety_timing.text.contains("Práce 80 % · chůze 100 %")
	...
	main._update_ui()
	_check(main.hud.production_detail_label.text.contains("Nosič #%d" % id)
		and main.hud.production_detail_label.text.contains("Rezerva jídla: 22 / 70 min při 1×")
		and main.hud.production_detail_label.text.contains("Jí Chléb"),
